...
*   **`origin`** (required; string, default empty): sender code, phone number or shortcode, that the messages will appear as being sent from; most providers will require this
*   **`show_sender`** (bool, default `true`): whether the sender is disclosed to the recipient
*   **`earliest_delivery`** (timestamp represented as epoch): do not deliver earlier than this timestamp; must indicate a timestamp in the future; if missing, schedule immediately; `mmsgw` will schedule sending at that time, and will pass the value on to other providers
*   **`latest_delivery`** (timestamp represented as epoch): do not deliver later than this timestamp; must indicate a timestamp in the future; missing means no lifetime limit; `mmsgw` will make sure the message metadata is preserved for at least that long, and will pass this parameter on to other providers; it is up to the providers to respect the restriction
*   **`expire_after`** (integer, seconds): it is ok not to maintain the message or its delivery information in the network anymore, after that many seconds; `mmsgw` will expire the message itself that many seconds, and will pass this parameter on to other providers; it is up to the providers to respect the requested behavior
...
*   **`subject`** (string): arbitrary text string that would be displayed above the message content
*   **`parts`** (required; list): individual message parts; each part in the list is a dictionary:
    ...
    *   **`content_type`** (required, string, see constants.py file for supported values): MIME type of the part; a part with a `content_type` of `application/smil` will always be forcefully pushed in the first position in the list 
...
On success, `mmsgw` replies with a full description of the template object created, in a JSON dictionary form. It also includes an unique identifier for the object created, and returns it as a dictionary member called `id`. If an ad-hoc send operation was requested, ??. 
...
*   **`gateway`** (string): the name of the gateway that this message needs to go out thru, which is one of the "`group`" names of the active gateways (e.g. if you have `GW01`, `GW02` and `GW03` as defined gateway instances, and the first two are both configured as part of the "`PROV-1`" group, use "`PROV-1`" as a value of this API parameter, and one of GW01 or GW02 will transmit your message); there is a default gateway parameter defined in the `mmsgw` settings, that will be adopted if this parameter is missing 
...
*   **`peer_ref`** (string): unique message ID attributed by the upstream provider; outbound messages get a value in this field after the provider acknowledged the message receipt
...
*   **`gateway`** (string): name of the group the gateway instance is part of
...
*   **`rr_requested`** (bool): whether the sender side requested to be notified of a read receipt event **`created_ts`** (integer, timestamp as epoch): timestamp of the message object creation and enqueuing
*   **`processed_ts`** (integer, timestamp as epoch): timestamp of when the message was processed from the queue
